...
# ---------------------------------------------------------------------------
# Agent decorators (exactly 3: @agent_model, @agent_config, @prompt_section)
# ---------------------------------------------------------------------------
        ...
        try:
            with tracer.start_as_current_span("pallet-verification-flow"):
                ...
                with tracer.start_as_current_span("M2-hu-label-detection"):
                    pass  # label detection happens via tool calls inside the graph

                with tracer.start_as_current_span("M3-delivery-data-fetch"):
                    pass  # EWM data fetch happens via MCP tool calls inside the graph

                with tracer.start_as_current_span("M4-hu-matching"):
                    pass  # HU matching happens via tool call inside the graph
