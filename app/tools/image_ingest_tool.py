...
def _validate_base64(image_data: str) -> tuple[bool, str | None, int]:
    ...
    if detected_mime and detected_mime not in SUPPORTED_MIME_TYPES:
        return False, f"Unsupported image format: {detected_mime}. Supported: {sorted(SUPPORTED_MIME_TYPES)}", size

    # Validate minimum size
    if size < _MIN_SIZE:
        return False, f"Image too small ({size} bytes). Minimum required: {_MIN_SIZE} bytes", size
...
def validate_and_prepare_image(image_data: str, source_channel: str) -> dict:
    ...
    """
    valid_channels = {"mobile", "handheld", "dock_camera", "web"}
    channel = source_channel if source_channel in valid_channels else source_channel

    image_id = str(uuid.uuid4())

    if not image_data or not image_data.strip():
        error = "No image data provided"
        logger.warning("M1.missed: pallet photo ingestion failed — invalid format or resolution below threshold")
